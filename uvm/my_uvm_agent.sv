...
class my_uvm_agent extends uvm_agent;
	...
	virtual function void build_phase( uvm_phase phase );
		...
		seqr = my_uvm_sequencer::type_id::create(
			.name( "seqr" ), .parent( this )
		);
		drvr = my_uvm_driver::type_id::create(
			.name( "drvr" ), .parent( this )
		);
		mon_out	= my_uvm_monitor_output::type_id::create(
			.name( "mon_out" ), .parent( this ) 
		);
